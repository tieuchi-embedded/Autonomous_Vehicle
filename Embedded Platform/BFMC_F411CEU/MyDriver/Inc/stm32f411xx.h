 ...
 ******************************************************************************
 * @file    stm32f411xx.h
 * @brief   STM32F411xx peripheral register definitions
 *          Based on RM0383 Rev3 - STM32F411xC/E Reference Manual
 *
 * Target: STM32F411CEU6 (Cortex-M4F, 100MHz, 512KB Flash, 128KB SRAM, UFQFPN48)
 *
 * Usage:
 ...
 *   - Max clock: 100 MHz (vs 168 MHz on F407)
 ...
 *   - Has SPI4 and SPI5 (vs F401 which only has SPI4 on xC/xE)
 *   - Flash latency: 0WS<=30MHz, 1WS<=64MHz, 2WS<=90MHz, 3WS<=100MHz
 *     (at VDD 2.7-3.6V)
 *
 * Key differences from STM32F401xx:
 *   - Max clock: 100 MHz (vs 84 MHz on F401)
 *   - Has SPI5 (F401 only has SPI4 on some variants)
 *   - Slightly different flash wait state thresholds
 ******************************************************************************
 */
...
/*===========================================================================
 * SECTION 2 — BUS BASE ADDRESSES (RM0383 Table 1, memory map)
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 3 — PERIPHERAL BASE ADDRESSES (RM0383 Table 1)
 *
 * NOTE: F411 does NOT have: TIM6, TIM7, TIM8, TIM12, TIM13, TIM14,
 *       DAC, CAN1, CAN2, UART4, UART5, ADC2, ADC3, GPIOF, GPIOG, GPIOI,
 *       Ethernet, DCMI, RNG.
 *       F411 HAS: SPI4 and SPI5 (extra vs F401).
 *===========================================================================*/
...
#define SPI5_BASE           (APB2_BASE + 0x5000U)   /* F411 only          */
...
#define GPIOH_BASE          (AHB1_BASE + 0x1C00U)   /* Only PA-PE, PH     */
...
/*===========================================================================
 * SECTION 4 — GPIO (RM0383 Chapter 8)
 *
 * Same register layout as F407/F401.
 * F411 has ports: A, B, C, D, E, H (no F, G, I)
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 5 — RCC (Reset and Clock Control) (RM0383 Chapter 6)
 *
 * Clock tree summary (F411):
 *   HSI  → 16 MHz RC
 *   HSE  → 4–26 MHz external crystal
 *   PLL  → (HSI/M or HSE/M) × N / P  (max 100 MHz SYSCLK)
 *          × N / Q → USB OTG FS (48 MHz required)
 *   AHB  → SYSCLK / HPRE        (max 100 MHz)
 *   APB1 → AHB / PPRE1          (max 50 MHz)
 *   APB2 → AHB / PPRE2          (max 100 MHz)
 *
 * NOTE: F411 has PLLI2S for I2S clocking.
 *       No PLLSAI, no AHB2RSTR/AHB3RSTR/AHB3ENR.
 *===========================================================================*/
...
#define RCC_APB2ENR_SPI5EN      (1U << 20)   /* F411 only                */
...
/*
 * PLLCFGR: PLL configuration (RM0383 §6.3.2)
 *   PLLM [5:0]  — VCO input divider   (2..63, typ 4 for 4MHz VCO input)
 ...
 *
 * Example: HSE=25MHz → PLLM=25, PLLN=400, PLLP=÷4 → SYSCLK=100MHz
 *                                          PLLQ=9  → ~44MHz (adjust as needed)
 * Example: HSI=16MHz → PLLM=8,  PLLN=200, PLLP=÷4 → SYSCLK=100MHz
 *                                          PLLQ=7  → ~57MHz (use PLLM=16, PLLN=192, PLLQ=4 for USB)
 */
#define RCC_PLLCFGR_PLLSRC_HSI (0U << 22)
#define RCC_PLLCFGR_PLLSRC_HSE (1U << 22)

/*===========================================================================
 * SECTION 6 — USART / UART (RM0383 Chapter 19)
 *
 * F411 has: USART1 (APB2), USART2 (APB1), USART6 (APB2)
 * No UART4, UART5 on F411.
...
/*===========================================================================
 * SECTION 7 — SPI (RM0383 Chapter 20)
 *
 * F411 has: SPI1 (APB2), SPI2 (APB1), SPI3 (APB1), SPI4 (APB2), SPI5 (APB2)
 * SPI5 is unique to F411 among the F4 Access Line.
 *===========================================================================*/
...
#define SPI5    ((SPI_TypeDef *) SPI5_BASE)   /* F411 only */
...
/*===========================================================================
 * SECTION 8 — I2C (RM0383 Chapter 18)
 *
 * F411 has: I2C1, I2C2, I2C3
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 9 — TIMERS (RM0383 Chapter 11-14)
 *
 * F411 timers:
 ...
 *
 * NOTE: No TIM6, TIM7, TIM8, TIM12, TIM13, TIM14 on F411.
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 10 — ADC (RM0383 Chapter 11)
 *
 * F411 has only ADC1 (no ADC2, no ADC3).
 * 12-bit SAR ADC, up to 2.4 MSPS.
 * ADC clock = APB2 / prescaler (max 36 MHz on F411).
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 11 — DMA (RM0383 Chapter 9)
 *
 * F411 uses stream-based DMA (same as F407/F401).
...
/*===========================================================================
 * SECTION 12 — EXTI (External Interrupt) (RM0383 Chapter 10)
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 13 — SYSCFG (RM0383 Chapter 7)
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 14 — PWR (Power Control) (RM0383 Chapter 5)
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 15 — FLASH Interface (RM0383 Chapter 3)
 *
 * Flash wait states (VDD 2.7–3.6V):
 *   0 WS : SYSCLK ≤ 30 MHz
 *   1 WS : SYSCLK ≤ 64 MHz
 *   2 WS : SYSCLK ≤ 90 MHz
 *   3 WS : SYSCLK ≤ 100 MHz  ← required at 100 MHz
 *
...
#define FLASH_ACR_LATENCY_3WS   (3U << 0)   /* Required at 100 MHz        */
...
/*===========================================================================
 * SECTION 16 — IWDG / WWDG (Watchdogs) (RM0383 Chapter 17-18)
 *===========================================================================*/
...
/*===========================================================================
 * SECTION 17 — CRC (RM0383 Chapter 4)
 *===========================================================================*/
