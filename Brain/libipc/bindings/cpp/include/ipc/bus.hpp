...
public:
    explicit Publisher(TopicId id, size_t payload_size = sizeof(T))
        : pub_(ipc_publish(id, payload_size)) {}
    ~Publisher() { ipc_unpublish(pub_); }

    Publisher(const Publisher&)            = delete;
    Publisher& operator=(const Publisher&) = delete;

    bool valid() const { return pub_ != nullptr; }
    int send(const T& msg) { return ipc_push(pub_, &msg, sizeof(T)); }
};
...
public:
    explicit Subscriber(TopicId id) : sub_(ipc_subscribe(id)) {}
    ~Subscriber() { ipc_unsubscribe(sub_); }
    ...
    std::optional<T> poll(int timeout_ms = 100) {
        T buf{};
        return ipc_pull(sub_, &buf, sizeof(T), timeout_ms) == 0
